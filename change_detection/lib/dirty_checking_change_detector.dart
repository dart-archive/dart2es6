...
class _MapChangeRecord<K, V> implements MapChangeRecord<K, V> {
  ...
  void _undoDeltas() {
    KeyValueRecord<K, V> rec;

    for (rec = _changesHead; rec != null; rec = rec._nextChanged) {
      rec._previousValue = rec._currentValue;
    }

    for (rec = _additionsHead; rec != null; rec = rec._nextAdded) {
      rec._previousValue = rec._currentValue;
    }

    assert((() {
      var rec = _changesHead;
      while (rec != null) {
        var nextRecord = rec._nextChanged;
        rec._nextChanged = null;
        rec = nextRecord;
      }

      rec = _additionsHead;
      while (rec != null) {
        var nextRecord = rec._nextAdded;
        rec._nextAdded = null;
        rec = nextRecord;
      }
      ...
      rec = _removalsHead;
      while (rec != null) {
        var nextRecord = rec._nextRemoved;
        rec._nextRemoved = null;
        rec = nextRecord;
      }
  ...
  void _truncate(KeyValueRecord lastRecord, KeyValueRecord record) {
    ...
    for (var rec = _removalsHead; rec != null; rec = rec._nextRemoved) {
      rec._previousValue = rec._currentValue;
  ...
  String toString() {
    ...
    KeyValueRecord<K, V> rec;
    for (rec = _mapHead; rec != null; rec = rec._next) {
      itemsList.add("$rec");
    }
    for (rec = _previousMapHead; rec != null; rec = rec._nextPrevious) {
      previousList.add("$rec");
    }
    for (rec = _changesHead; rec != null; rec = rec._nextChanged) {
      changesList.add("$rec");
    }
    for (rec = _additionsHead; rec != null; rec = rec._nextAdded) {
      additionsList.add("$rec");
    }
    for (rec = _removalsHead; rec != null; rec = rec._nextRemoved) {
      removalsList.add("$rec");
    }
    return
        "map: ${itemsList.join(", ")}\n"
